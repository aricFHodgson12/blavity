...
export default {
  ...
  computed: {
    ...
    title () {
      return he.decode(this.tile.title)
    },
    subtitle () {
      return he.decode(this.tile.subTitle)
    },
    image () {
      if (this.tile.wp_featuredImage.includes('//blavity.com/')) {
        ...
        if (this.isViewportMobile) {
          return `https://res.cloudinary.com/blavity/image/upload/c_fit,g_center,w_300,q_auto:best,g_south_east,x_0/${this.tile.wp_featuredImage}`
        }
        return `https://res.cloudinary.com/blavity/image/upload/c_fit,g_center,w_600,q_auto:best,g_south_east,x_0/${this.tile.wp_featuredImage}`
      }
    },
    views () {
      if (this.tile.views) {
        return this.tile.views
      } else {
        return '0'
      }
    }
