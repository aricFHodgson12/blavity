        ...
        <div class="grid-container">
          <div class="grid-row">
            ...
            <main>
              ...
              <div class="bo-page-content__article-body">
                <div
...
import ArticleBoHeader from '~/components/ArticleBOHeader'
...
export default {
  ...
  async asyncData ({ app, error, params, store, redirect }) {
    ...
    try {
      ...
      if (article) {
        const checkForBlavityOriginalTag = article.tags.map(function (element) {
          if (element.slug === 'blavity-original' || element.slug === 'Blavity-Original' || element.slug === 'blavity-Original' || element.slug === 'Blavity-original') {
            return 'original'
          } else return 'not-original'
        })
        if (checkForBlavityOriginalTag.indexOf('original') === - 1) {
  ...
  methods: {
    ...
    image (post) {
      if (post.wp_featuredImage.includes('//blavity.com/')) {
        return post.wp_featuredImage.replace('blavity.com', 'legacy.blavity.com')
      } else if (post.wp_featuredImage.includes('res.cloudinary.com')) {
        return post.wp_featuredImage
      } else if (post.wp_featuredImage.includes('http')) {
        return post.wp_featuredImage
      } else {
        if (this.isViewportMobile) {
          return `https://res.cloudinary.com/blavity/image/upload/c_fit,g_center,h_250,q_auto:best,g_south_east,x_0/${post.wp_featuredImage}`
        }
        return `https://res.cloudinary.com/blavity/image/upload/c_fill,g_center,w_800,h_500,q_auto:best,g_south_east,x_0/${post.wp_featuredImage}`
      }
    },
    twitterLoad () {
      /* eslint-disable */
